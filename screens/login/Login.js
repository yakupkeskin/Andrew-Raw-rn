...
export default function Login({navigation}) {
  ...
  useEffect(() => {
    ...
    setRedux();
    setFirst(1); // Whenever token changes we update the redux.
  }, [token]);
  ...
  function setRedux(){
    ...
    console.log("Login setted",token,userName)
    }
    
  }

  function login(){
    axios.post(url+"/auth/token/login/",{"username":userName,"password":password}) //Login request
